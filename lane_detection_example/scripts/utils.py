...
class CURVEFit:
    ...
    def fit_curve(self, lane_pts):
        ...
        if len(y_left)==0 or len(y_right)==0:
            self._init_model()
            x_right, y_left, x_right, y_right = self.preprocess_pts(lane_pts)
...
class BEVTransform:
    ...
    def build_tf(self, params_cam):
        ...
        self.proj_mtx = projectZimg_mtx(params_cam)
        xn, yn = xc/zc, yc/zc
    ...
    def project_pts2img(self, xyz_bird):
        ...
        xyl = np.matmul(self.proj_mtx, np.concatenate([xn, yn, np.ones_like(xn)], axis=0))

        xyl = xyl[0:2,:].T
        return xyl
    def crop_pts(self, xyl):
        xyl = xyl[np.logical_and(xyl[:,0]>=0, xyl[:,0]<self.width), :]
        xyl = xyl[np.logical_and(xyl[:,1]>=0, xyl[:,1]<self.height), :]
...
def projectZimg_mtx(params_cam):
    #project the lidar point to 2d plane
    #\n xc, yc, zc : xyz components of lidar points w,r,t a camera coordinate
    #\n params_cam: parameters from camera_calibration_parsers
